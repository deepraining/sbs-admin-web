...
export function isPrice(str) {
  ...
  // 1. / 1.234
  if (strArray.length > 1 && (!strArray[1] || !strArray[1].length || strArray[1].length > 2))
    return false;
...
export function isLongitude(str) {
  ...
  if (strArray.length > 2) return false;
  if (Math.abs(parseFloat(strArray[0])) > 180) return false;
...
export function isLatitude(str) {
  ...
  if (strArray.length > 2) return false;
  if (Math.abs(parseFloat(strArray[0])) > 90) return false;
