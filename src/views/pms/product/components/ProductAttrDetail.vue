...
export default {
  ...
  methods: {
    ...
    handleAddProductAttrValue(idx) {
      const { options } = this.selectProductAttr[idx];
    ...
    handleSyncProductSkuPrice() {
      this.$confirm('将同步第一个sku的价格到所有sku,是否继续', '提示', {
        ...
        if (
          ...
          const { price } = this.value.skuStockList[0];
    ...
    handleSyncProductSkuStock() {
      this.$confirm('将同步第一个sku的库存到所有sku,是否继续', '提示', {
        ...
        if (
          ...
          const { stock } = this.value.skuStockList[0];
          const { lowStock } = this.value.skuStockList[0];
          for (let i = 0; i < tempSkuList.length; i++) {
    ...
    refreshProductAttrPics() {
      ...
      if (this.selectProductAttr.length >= 1) {
        const { values } = this.selectProductAttr[0];
        for (let i = 0; i < values.length; i++) {
          ...
          this.selectProductAttrPics.push({ name: values[i], pic });
        }
      }
    },
    // 获取商品相关属性的图片
